...
class Cov_block(nn.Module):                 #定义卷积模块
    def __init__(self,in_chanels,med_chanels,out_chanels):
        ...
        self.Cov = nn.Sequential(
            nn.Conv2d(in_chanels, med_chanels, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(med_chanels),
            nn.ReLU(),
            nn.Conv2d(med_chanels, out_chanels, kernel_size=3, stride=1, padding=1),
    ...
    def forward(self,x):
        y = self.Cov(x)
        return y
def Copy_Crop(feature1,feature2):
...
class Get_net(nn.Module):
    def __init__(self):
        ...
        self.Up_Conv1 = Cov_block(512, 256, 256)
        self.Up_Sample2 = Up_Sample(256, 128)
        self.Up_Conv2 = Cov_block(256, 128, 128)
        self.Up_Sample3 = Up_Sample(128, 64)
        self.Up_Conv3 = Cov_block(128, 64, 64)
        self.One_point_One = nn.Conv2d(64,1,kernel_size=1,stride=1,padding=0)
        self.sig = nn.Sigmoid()
    def forward(self,x):
        ...
        out = self.Up_Sample1(y4)
        out = self.Up_Conv1(Copy_Crop(y3,out))
        out = self.Up_Sample2(out)
        out = self.Up_Conv2(Copy_Crop(y2, out))
        out = self.Up_Sample3(out)
        out = self.Up_Conv3(Copy_Crop(y1, out))
